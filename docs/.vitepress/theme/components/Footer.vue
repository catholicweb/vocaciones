      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
        ...
        <div>
          ...
          <div class="flex gap-4 items-center mx-auto items-center justify-center">
            <a v-if="config.social.youtube" target="_blank" rel="noopener noreferrer" :href="config.social.youtube" class="inline-block hover:text-red-500 transition-colors">
              <svg class="w-10 h-10" fill="currentColor" viewBox="0 0 24 24">
                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
              </svg>
            </a>

            <a v-if="config.social.instagram" target="_blank" rel="noopener noreferrer" :href="config.social.instagram" class="inline-block hover:text-pink-500 transition-colors">
            ...
            <a v-if="config.social.twitter" target="_blank" rel="noopener noreferrer" :href="config.social.twitter" class="inline-block hover:text-gray-700 transition-colors">
              <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
            </a>

            <a v-if="config.social.tiktok" target="_blank" rel="noopener noreferrer" :href="config.social.tiktok" class="inline-block hover:text-[#ff0050] transition-colors">
              <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
                <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z" />
              </svg>
            </a>

            <a v-if="config.social.twitch" target="_blank" rel="noopener noreferrer" :href="config.social.twitch" class="inline-block hover:text-purple-500 transition-colors">
              <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
                <path d="M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714Z" />
              </svg>
            </a>

            <a v-if="config.social.facebook" target="_blank" rel="noopener noreferrer" :href="config.social.facebook" class="inline-block hover:text-blue-600 transition-colors">
              <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
